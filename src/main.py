...
def main():
    if len(sys.argv) < 2:
        print("Usage: python script.py <file_name> [sheet_name] [question_column] [answer_column]")
        return

    file_name = sys.argv[1]

    if file_name.endswith('.pdf'):
        questions = extract_questions_from_pdf(file_name)
        answers = []
        for q in questions:
        ...
        if len(sys.argv) != 5:
            print("Usage for Excel files: python script.py <file_name> <sheet_name> <question_column> <answer_column>")
            return
        sheet_name = sys.argv[2]
        question_column = sys.argv[3]
        answer_column = sys.argv[4]
        questions = fill_out_xlsx(file_name, sheet_name, question_column, answer_column)
        answers = []
        for q in questions:
            answers.append(get_best_answer(q))

    elif file_name.endswith('.png'):
        questions = get_questions_from_image(file_name)
        answers = []
        for q in questions:
            answers.append(get_best_answer(q))
    else:
